...
let dataBuyerPubkey: PublicKey;

let dataBuyerAccount;
...
class DataBuyerAccount {
    buyer_account = "Acz3XbJH63t232xMy8t6jw3soq7gCDUZqHe5io7Y7b1a";
    amount = 0;
    constructor(fields: {buyer_account: string, amount: number} | undefined = undefined) {
        if (fields) {
            this.buyer_account = fields.buyer_account;
            this.amount = fields.amount;
        }
    }
}

const DataBuyerAccountSchema = new Map([
    [DataBuyerAccount, {kind: 'struct', fields: [['buyer_account', 'String'], ['amount', 'u64']]}],
]);

const Data_Buyer_SIZE = borsh.serialize(
    DataBuyerAccountSchema,
    new DataBuyerAccount(),
).length;
...
export async function checkAccounts(): Promise<void> {
    ...
    const INDEX_SEED = 'amoebit_minterr';
    const INDEX_SEED2 = 'buy11212111'
    ...
    dataBuyerPubkey = await PublicKey.createWithSeed(
        payer.publicKey,
        INDEX_SEED2,
        programId,
    )

    dataBuyerAccount = await connection.getAccountInfo(dataBuyerPubkey);
    if (dataBuyerAccount === null) {
        console.log(
            'Creating account',
            dataBuyerPubkey.toBase58(),
            'to count store struct buyer',
        );
        const lamports = await connection.getMinimumBalanceForRentExemption(
            Data_Buyer_SIZE,
        );
        
        const transaction17 = new Transaction().add(
            SystemProgram.createAccountWithSeed({
                basePubkey: payer.publicKey,
                fromPubkey: payer.publicKey,
                lamports,
                newAccountPubkey: dataBuyerPubkey,
                programId: programId,
                seed: INDEX_SEED2,
                space: Data_Buyer_SIZE,
            }),
        );
        await sendAndConfirmTransaction(connection, transaction17, [payer]);
    }
...
export async function testContract(): Promise<void> {
    ...
    account_15    = {pubkey: dataBuyerPubkey,   isSigner: false, isWritable: true},
    account_16    = {pubkey: new PublicKey("9ajQyTZn525TK5e8dXLzaWLsX5FHCSyo2VVszpjmXQCb"),  isSigner: false, isWritable: true};
