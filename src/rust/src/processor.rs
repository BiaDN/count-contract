...
pub fn process_instruction<'a>(
    ...
    let data_buyer_account     = next_account_info(accounts_iter)?; // 15
    let buyer_account          = next_account_info(accounts_iter)?; // 16
    ...
    let mut data_buyer = Buyer::try_from_slice(&data_buyer_account.data.borrow())?;


    if data_buyer.buyer_account.to_string() != buyer_account.key.to_string() {
    ...
    data_buyer.amount += 100;
    series_index.serialize(&mut &mut index_account.data.borrow_mut()[..])?;
    series_index.serialize(&mut &mut data_buyer_account.data.borrow_mut()[..])?;
